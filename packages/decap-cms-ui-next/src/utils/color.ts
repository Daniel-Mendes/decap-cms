...
function lighten(colorString, strength) {
  return Color(colorString).mix(Color('white'), strength).saturate(strength).hex();
}

function darken(colorString, strength) {
  const { h } = Color(colorString).hsl().object();
  const hueShift =
    0 <= h && h <= 180
  ...
  return Color(colorString).mix(Color('black'), strength).saturate(strength).rotate(hueShift).hex();
}

function getShadeValue(pos, steps) {
...
function getShadeColor(colorString, pos) {
